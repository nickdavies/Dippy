        ...
        for part in parts:
            ...
            if part_kind == "procsub":
                ...
                if (
                    ...
                    outer_result = handler.classify(HandlerContext(words[base_idx:], config=config))
...
def _analyze_simple_command(
    ...
    *,
    ...
    handler = get_handler(base)
    if handler:
        result = handler.classify(HandlerContext(tokens, config=config, word_has_expansions=word_has_expansions))
